...
static NSString *token=@"Dddi23*DOO#LKD3";
static NSString *signStr=@"sn";
static NSString *timeStr=@"tm";
static NSString *randStr=@"ds";
static NSString *bodyStr=@"by";
...
- (void)doLogin
{
    ...
    [Param setObject:loginAccount forKey:@"username"];
    [Param setObject:loginToken forKey:@"password"];
    [Param setObject:@"0" forKey:@"account_type"];
    [Param setObject:@"86" forKey:@"ccode"];
    
    AFHTTPSessionManager *mgr=[AFHTTPSessionManager manager];
    mgr.responseSerializer = [AFHTTPResponseSerializer serializer];
    NSDictionary *dic = [self secret];
    [mgr.requestSerializer setValue:@"1.0.0" forHTTPHeaderField:@"ver"];
    [mgr.requestSerializer setValue:[dic objectForKey:@"sign"] forHTTPHeaderField:signStr];
    [mgr.requestSerializer setValue:[dic objectForKey:@"time"] forHTTPHeaderField:timeStr];
    [mgr.requestSerializer setValue:[dic objectForKey:@"rand"] forHTTPHeaderField:randStr];
    [mgr.requestSerializer setValue:[self setBodyParam:Param] forHTTPHeaderField:bodyStr];
    ...
    NSString *url = @"https://api.everpoker.win/user/login";
    [mgr POST:url parameters:Param progress:^(NSProgress * _Nonnull downloadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        [SVProgressHUD dismiss];
        NSDictionary *result = [NSJSONSerialization JSONObjectWithData:responseObject options:0 error:NULL];
        if ([result[@"code"]intValue] == 0) {
            [[[NIMSDK sharedSDK] loginManager] login:result[@"data"][@"uid"]
            ...
            [self.view makeToast:[NSString stringWithFormat:@"登录失败，code：%@", result[@"code"]] duration:5.0 position:CSToastPositionCenter];
        }
...
- (NSDictionary *)secret {
    ...
    NSString *randStr=[NSString stringWithFormat:@"%d",x];
    NSArray *array=[NSArray arrayWithObjects:timeString,token,randStr, nil];
    NSArray *array2 = [array sortedArrayUsingSelector:@selector(compare:)];
    NSString *signStr=[[array2 componentsJoinedByString:@""] sha1];
    //    NLog(@"signStr===%@",signStr);
    return [NSDictionary dictionaryWithObjectsAndKeys:signStr,@"sign",timeString,@"time",randStr,@"rand", nil];
}

- (NSString *)setBodyParam:(NSMutableDictionary *)param {
    NSArray *array=[NSDictionary dictionaryWithDictionary:param].allKeys;
    NSArray *array2 = [array sortedArrayUsingSelector:@selector(compare:)];
    NSString *str = token;
    for (int i=0; i<array2.count; i++) {
        NSString *key=[array2 objectAtIndex:i];
        str=[NSString stringWithFormat:@"%@%@%@",str,key,[param objectForKey:key]];
    }
    //    NLog(@"%@,%@",str,[str MD5Hash]);
    return [str MD5Hash];
}
